    ...
    socket.on("newmessage", (newMessage) => {
      setChats(prevChats => {
        const chatExists = prevChats.some(chat => 
          chat.users.some(u => u._id === newMessage.senderId) && 
          chat.users.some(u => u._id === newMessage.receiverId)
        );

        if (chatExists) {
          return prevChats.map(chat => {
            const isChatMatch = chat.users.some(u => u._id === newMessage.senderId) && 
                                chat.users.some(u => u._id === newMessage.receiverId);
            if (isChatMatch) {
              return { ...chat, latestMessage: newMessage, updatedAt: new Date().toISOString() };
            }
            return chat;
          }).sort((a, b) => new Date(b.updatedAt) - new Date(a.updatedAt));
        } else {
          // If chat doesn't exist in list (new chat), fetch all chats to get the populated data
          fetchChats();
          return prevChats;
        }
      });
